<route lang="yaml">
              ...
              <el-scrollbar height="400px">
                <div v-for="(children, ind) in obj4">
                  ...
                  >
                    ...
                    <el-input v-model="obj4[ind]"  @click="changeGoods(children, ind, obj2)" placeholder="Please input" />
                  </div>
                </div>
              </el-scrollbar>
            </div>
  ...
  const editClick = (item: any) => {
    if (item.data_type == "static" && item.type == "ax001") {
      ...
      console.log(item, "44444444444444444");
      show.value = 3;
      obj4.value = item;
      console.log(obj4.value, "44444444444444444");
      dialogVisible.value = true;
    } else if (item.data_type == "sql" && item.type == "ax002") {
  ...
  const change1 = (e: any) => {
    ...
    console.log(e.target.files[0], 123123);
    // 手写的input需要一个对象将本地图片转换为对应的格式来上传
    let formData = new FormData();
    // e.target.files就是选中的文件的一个数组
    formData.append("file", e.target.files[0]);
