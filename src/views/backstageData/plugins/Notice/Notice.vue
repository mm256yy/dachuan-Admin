<route lang="yaml">
    ...
    function getlist() {
      ...
      api
        ...
        .then((res: any) => {
          // console.log(res,'公告列表数据66666')
          tableData.value = res.body.list;
          console.log(tableData.value,'公告列表数据')
          total.value = res.body.total;
        });
    }
    function changeshowHide(val:any){
      // const data=tableData.value.filter((item:any)=>item.id==val);
     console.log(typeof val,'状态6666')
     
     http.post("/api/plugs/updatePlugsNotice",val)
                .then((res: any) => {
                  if (res.code == 200) {
                    ElMessage.success({
                      message: "修改成功状态成功",
