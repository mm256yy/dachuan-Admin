              ...
              >
                ...
                <el-table-column fixed="right" label="操作" width="80">
                  <template #default="scope">
                    <el-button
                      type="danger"
                      @click="delPlugsRebate(handleClick(scope.row.id))"
                      >删除</el-button
                    >
                  </template>
                </el-table-column>
          ...
          <div class="content">
            ...
            <div class="main">
              ...
              >
                ...
                <el-table-column label="好友返利	" align="center">
                  <template #default="scope">
                    <div>{{ scope.row.friendRebate }}</div>
                  </template>
                </el-table-column>

                <el-table-column prop="orderNo" label="分享返利	" align="center">
                  <template #default="scope">
                ...
                <el-table-column prop="orderNo" label="状态" align="center">
                  <template #default="scope">
                    <div>
                      <el-switch
                ...
                <el-table-column label="操作" align="center">
                  <template #default="scope">
                    <div>
                      <el-button
                        type="danger"
                        @click="delPlugsRebate(scope.row)"
                        >删除</el-button
                      >
                    </div>
                  </template>
          ...
          <div class="content">
            ...
            <div class="main">
              ...
              >
                ...
                <el-table-column prop="orderNo" label="提现标题" align="center">
                  <template #default="scope">
                    <div>{{ scope.row.withdrawTitle }}</div>
                  </template>
                </el-table-column>

                <el-table-column prop="orderNo" label="提现类型" align="center">
                  <template #default="scope">
                    <div>
                      <div v-if="scope.row.withdrawType == 0">支付宝</div>
                      <div v-else-if="scope.row.withdrawType == 1">微信</div>
                      <div v-else-if="scope.row.withdrawType == 1">银行卡</div>
                    </div>
                  </template>
                </el-table-column>

                <el-table-column prop="orderNo" label="提现信息	" align="center">
                  <template #default="scope">
                    <div>
                      <div v-if="scope.row.withdrawType == 0">
                        <div>
                          {{
                            JSON.parse(scope.row.withdrawInfo)["userAlipay"][
                              "name"
                            ]
                          }}
                        </div>
                      ...
                      <div v-else-if="scope.row.withdrawType == 2">
                        <div>
                          {{
                            JSON.parse(scope.row.withdrawInfo)["bank_car"]
                              .bankCardNo
                          }}
                        </div>
                        <div>
                ...
                <el-table-column prop="orderNo" label="提现状态" align="center">
                  <template #default="scope">
                    <div>
                      <div v-if="scope.row.withdrawStatus == 1">未打款</div>
                ...
                <el-table-column prop="orderNo" label="	审核状态" align="center">
                  <template #default="scope">
                    <div>
                      <div v-if="scope.row.auditStatus == 1">待审核</div>
                      <div v-else-if="scope.row.auditStatus == 2">审核成功</div>
                      <div v-else-if="scope.row.auditStatus == 3">审核失败</div>
                    </div>
                  </template>
                </el-table-column>
...
const shenhe = (item: any) => {
  ElMessageBox.confirm(`确认审核成功吗？`, "确认").then(() => {
    item.auditStatus = 2;
    http
      .post("/api/plugs/updatePlugsRebateWithdrawalRecord", item)
      .then((res: any) => {
        if (res.code == 200) {
          ElMessage.success({
            message: res.msg,
...
const deleteTixian = (item: any) => {
  ElMessageBox.confirm(`确认删除吗？`, "确认信息").then(() => {
    ...
    api
      .post("/api/plugs/delPlugsRebateWithdrawalRecord", data1)
      .then((res: any) => {
        if (res.code == 200) {
          get_tixian();
          ElMessage.success({
...
const total1: any = ref(0);
const total2: any = ref(0);
const get_fenxiao = () => {
  const data1: any = {
  ...
  http
    .get("/api/plugs/searchPlugsRebateList", {
      params: data1,
    })
    .then((res: any) => {
      if (res.code == 200) {
...
const get_tixian = () => {
  ...
  http
    .get("/api/plugs/searchPlugsRebateWithdrawalRecordList", {
      params: data1,
    })
    .then((res: any) => {
