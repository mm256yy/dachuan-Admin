...
const form = ref({
  ...
  bussinessId: 0,
  friendRebate: 0,
  selfPurchaseRebate: 0,
  shareRebate: 0,
  state: 0,
  totalFriendRebate: 0,
  totalSelfPurchaseRebate: 0,
  totalShareRebate: 0,

  withdrawFriendRebate: 0,
  withdrawSelfPurchaseRebate: 0,
  withdrawShareRebate: 0,
});
  ...
  if (form.value.id === "") {
    formRef.value &&
      formRef.value.validate((valid: any) => {
        if (valid) {
          api
            .post("/api/plugs/insertPlugsRebate", form.value)
            .then((res: any) => {
              ElMessage.success({
                message: "新增成功",
                center: true,
    ...
    formRef.value &&
      formRef.value.validate((valid: any) => {
        if (valid) {
          api.post("/api/plugs/delPlugsRebate", form.value).then((res: any) => {
            if (res.code == 200) {
              ElMessage.success({
                message: "修改成功",
                center: true,
