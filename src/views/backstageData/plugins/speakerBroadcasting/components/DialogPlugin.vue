      ...
      >
        <div>
          <div  style="margin: 20px;height: 500px;overflow-y:scroll">
            ...
            <div style="display: flex;flex-direction: column;border-radius: 16px;border: 1px solid #eee;margin-top: 5px;" v-for="(item,index) in data.plugsConfigJson">
              <el-input

                  style="margin-left: 20px;width:90%;position: relative;top: 15px;"
                  v-model="item.device"
              ...
              <el-input
                ...
                >
                <template #prepend>
                  <el-select v-model="item.businessId" placeholder="选择店铺" style="width: 115px">
                    <div v-for="business in businessList">
                      <el-option :label="business.jsonViewData.businessName" :value="business.jsonViewData.businessId" />
                    </div>
                  </el-select>
                </template>
              </el-input>
...
function getDianpu() {
  api.get("/api/plugs/getBusinessInfo", { params: { size: 100 } })
    .then((res: any) => {
      if (res.code == 200) {
