<route lang="yaml">
    ...
    <div class="container">
      <div class="content">
        <div class="header" style="margin-bottom: 10px">
          ...
          <div style="display: flex">
            <div class="lang" style="display: flex;align-items: center;"  >
              <el-date-picker style="width: 352px;margin-right: 10px;" size="default" v-model="value2" @change="changeTime2"
							value-format="YYYY-MM-DD HH:mm:ss" type="datetimerange" range-separator="-"
							start-placeholder="开始日期" end-placeholder="结束日期" />
              <el-select style="width:180px;margin-right:15px" v-model="businessId" filterable placeholder="选择店铺">
        ...
        >
          ...
          >
          <el-table-column  label="用户ID" align="center" type="index" prop="userId" width="70"/>
            <el-table-column
              label="用户名"
              align="center"
        width="200"
  ...
  " >
              <div v-if="scope.row.businessId==item.businessId" >{{ item.businessName}}</div>
            </div>
      ...
      api.get("/api/plugs/getBusinessInfo").then((res: any) => {
        if (res.code == 200) {
          res.body.forEach((item: any) => {
            ...
            console.log(businessList.value,'店铺列表88888888')
          });
        }
      });
    }
  ...
  function rechargeRecord(){
    ...
    api.get('/api/pay/searchPayRecordList',{ params: data }).then((res:any)=>{
      if(res.code==200){
        ...
        console.log(recordList.value,'店铺列表4444444444444')
