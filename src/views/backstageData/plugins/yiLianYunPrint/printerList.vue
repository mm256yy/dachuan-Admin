    ...
    <div class="content" style="border-radius: 10px">
      ...
      <div class="main">
        ...
        >
          ...
          <!-- <el-table-column prop="orderId" label="id" width="200" align="center">
          ...
          <el-table-column prop="orderUserId" label="设备编码	" align="center">
            <template #default="scope">
              <div>{{ scope.row.machineCode }}</div>
            </template>
          </el-table-column>
          <el-table-column prop="printName" label="打印机名称	" align="center">
            <template #default="scope">
              <div>{{ scope.row.printName }}</div>
            </template>
          </el-table-column>
          <el-table-column prop="msign" label="打印机密钥" align="center">
            <template #default="scope">
              <div>{{ scope.row.msign }}</div>
            </template>
          </el-table-column>
        ...
        if (res.body) {
          try {
            ...
            if (JSON.parse(res.body.plugsConfigJson)[0].businessId) {
              data_list.value.plugsConfigJson = JSON.parse(
                res.body.plugsConfigJson
              );
            } else {
              data_list.value.plugsConfigJson = JSON.parse(
                res.body.plugsConfigJson
