    ...
    <div class="container">
      <div style="padding:20px;margin: 20px;background: white;">
        <div style="display: flex;flex-direction: column;">
            ...
            <div style="display: flex;flex-direction: column;padding: 10px 0;" v-for="(item,index) in menuData">
              <div style="font-weight: bolder;font-size: 20px;">{{ item.menuName }}</div>
              ...
              <div v-for="(item1,index1) in item.menusList" style="margin: 10px;">
                ...
                <div style="display: flex;margin:10px 10px;">
                  <div style="display:flex ;"  v-for="(tag,tagIndex) in item1.buttonList">
                  ...
                  <div v-if="buttonadd.buttonsName && buttonadd.id && buttonadd.menusId != 0">
                      <!-- {{ item1.input }} -->
                      <el-input
                    ...
                    />
                    <el-button style="margin: 5px;" v-else class="button-new-tag ml-1" size="small" @click="showInput(item1,index1)">
                      + 新增按钮
                    </el-button>
                  </div>
                </div>
...
const menuButton = ()=>{
  ...
  api.get('/api/admin/searchPowerMenusList').then((res:any)=>{
    ...
    if(res.code == 200){
      ...
      for (let index = 0; index < list.length; index++) {
        ...
        for (let index1 = 0; index1 < list[index].menusList.length; index1++) {
  ...
  // api.get('/api/admin/searchPowerButtonsList').then((res:any)=>{
  //   console.log(res)
  //   if(res.code == 200){
...
const handleClose1 = (tag:any,index:any,index1:any,tagIndex:any) => {
  ...
  menuData.value[index].menusList[index1].buttonList.splice(menuData.value[index].menusList[index1].buttonList.indexOf(tag),1)
  ...
  if(buttondelet.value.id && buttondelet.value.buttonsName && buttondelet.value.menusId){
...
const handleInputConfirm = (item:any) => {
  ...
  if (item.input) {
    ...
    console.log(inputValue.value,SavaData.value)
...
const shot = (item:any) =>{
  ...
  if(data.value.menuTitle == '确定'){
    ...
    http.post('/api/admin/insertPowerButtons',data.value.menuList).then((res:any)=>{
    if(res.code == 200){
      ElMessage({
        type:"success",
        message:'添加成功!'
    ...
    console.log(data.value.menuList)
