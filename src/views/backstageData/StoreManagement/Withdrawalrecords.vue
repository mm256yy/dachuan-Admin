<template>
      ...
      <div class="content">
        <div class="header" style="margin-bottom: 20px">
          ...
          <div style="display: flex">
            <el-select  style="width: 100px;margin-right: 6px;" v-model="Withdrawalstatus" @change="changeStatus" placeholder="提现状态"  >
        ...
        <div class="main">
          ...
          >
            ...
            <el-table-column prop="title" label="提现状态" align="center">
              <template #default="scope">
                <el-select v-if="scope.row.withdrawStatus===0 || scope.row.withdrawStatus===2" style="width: 100px;" v-model="scope.row.withdrawStatus"  placeholder="提现状态"  @change="value=>{changeState(value,scope.row)}"  >
                      <el-option :key="0" label="待审核" :value="0" />
                      <el-option :key="1" label="提现成功" :value="1" />
                      <el-option :key="2" label="提现失败" :value="2" />
               </el-select>
               <el-select v-else style="width: 100px;" v-model="scope.row.withdrawStatus" disabled  placeholder="提现状态"  >
                      <el-option :key="0" label="待审核" :value="0" />
                      <el-option :key="1" label="提现成功" :value="1" />
                      <el-option :key="2" label="提现失败" :value="2" />
               </el-select>
      ...
      .then((res: any) => {
        if (res.code == 200) {
         ...
         tableData.value.forEach((item:any )=> {
          ...
          // console.log(tableData.value,'提现记录')
  ...
  function changeState(val:any,val2:any){
        // console.log(val,'信息66666')
        // console.log(val2,'信息44444')
        const data={
        ...
        ElMessageBox.confirm(`确认修改此记录吗？`, "提现提示").then(()=>{
