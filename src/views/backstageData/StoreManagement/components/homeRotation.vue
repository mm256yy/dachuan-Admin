<template>
	<el-dialog v-model="myVisible" :title="title" width="800px" :close-on-click-modal="false" :show-close="false"
		...
		<div class="content">
			<el-form autocomplete="off" ref="formRef" :model="form" :rules="formRules" label-width="110px">
				<div>
					...
					<el-form-item label="用户权限" prop="businessListJson">
						<el-select :clearable="true" style="width:400px;margin-right:15px" @change="selectBusiness" multiple v-model="selectBuiness" filterable placeholder="选择店铺">
...
<script lang="ts" setup>
	...
	const title = computed(() => (props.id === "" ? "新增员工账号" : "修改员工账号"));
	...
	// console.log( storage.local.get('account'),'账号5555555555')
	// 新增充值余额
	const recharge=ref({
		"rechargeMoney":'', 
		"giveeMoney":'',
	  ...
	  api
	    ...
	    .then((res: any) => {
			if(res.body){
				...
				// console.log(selectBuiness.value,'员工权限列表66666')			
			}
	    });
	}
	function onSubmit() {
