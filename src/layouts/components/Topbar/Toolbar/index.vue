...
const breadcrumbList = computed(() => {
  ...
  if (route.name !== 'home' && settingsStore.settings.breadcrumb.enableMainMenu && !['single'].includes(settingsStore.settings.menu.menuMode)) {
    const index = menuStore.allMenus.findIndex((item:any) => item.children.some((r:any) => route.fullPath.indexOf(`${r.path}/`) === 0 || route.fullPath === r.path))
    ...
    <div class="left-box">
      ...
      >
        <transition-group name="breadcrumb">
          <el-breadcrumb-item v-for="(item, index) in breadcrumbList" :key="JSON.stringify(item)" :to="index < breadcrumbList.length - 1 && item.path !== '' ? pathCompile(item.path) : ''">
            {{
