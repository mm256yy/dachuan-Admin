    ...
    function flatAsyncRoutes<T extends RouteRecordRaw>(routes: T): T {
      if (routes.children) {
        routes.children = flatAsyncRoutesRecursive(routes.children, [{
          ...
          activeIcon: routes.meta?.activeIcon,
      ...
      if (settingsStore.settings.app.routeBaseOn !== 'filesystem') {
        if (routes.value) {
          routes.value.forEach((item) => {
            returnRoutes.push(...cloneDeep(item.children) as RouteRecordRaw[])
          })
          returnRoutes.forEach(item => flatAsyncRoutes(item))
        }
      }
      else {
        returnRoutes.push(...cloneDeep(fileSystemRoutes.value) as RouteRecordRaw[])
      }
      return returnRoutes
    })
